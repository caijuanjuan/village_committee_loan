...
import locale
import os
from utils import check_dir, save_txt, get_txt, check_res_ok
...
data_dir = "data/北京"  # 根据情况修改
save_dir = 'res/北京'  # 根据情况修改
...
for dir_name in dir_names:
    ...
    for file_name in file_names:  # 按重命名后的数字排序
        ...
        if not os.path.exists(save_path):  # 若结果不存在，才进行解析
            ...
            if not check_res_ok(save_path, pre_res):  # 说明结果没有生成对，重新来
                wrong_result_num += 1
